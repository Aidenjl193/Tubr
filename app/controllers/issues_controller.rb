class IssuesController < ApplicationController
  ...
  def create
    ...
    if params["line_id"] && params["station_id"]
      station_line_id = StationLine.find_by(line_id: params["line_id"], station_id: params["station_id"]).id
    end
    #forgot to add 2nd station_line_id to params...workaround:
    if params["second_station_line_id"] && params["line_id"]
      second_station_id = Station.find_by(name: params["second_station_line_id"]).id
      second_station_line_id = StationLine.find_by(line_id: params["line_id"], station_id: second_station_id).id
    end
