		...
		.then(json => {
			...
			const svgContainer = d3.select("#tube-overlay")
				  ...
				  .call(d3.zoom().on("zoom", () => {
					  ...
					  lables
						  .attr('x', function(d) {return newX(d.nodes[0].x)})
						  .attr('y', function(d) {return newY(d.nodes[0].y)});
			...
			let lines = svgContainer.selectAll("g")
				.data(json)
				.enter()
				.append("g")
			...
			const station_Joins_bottom = svgContainer.selectAll(".join-line-bottom")
				  .data(joins)
				  .enter()
				  .append("line")
